...
func NewDBClientAndConnection(ctx context.Context, env *internal.Environ) (*mongo.Client, *mongo.Database) {
	...
	if err != nil {
		log.Fatalf("[FATAL] Failed to create DB client: %s", err)
	}

	// Check that the database connection has been established
	err = dbClient.Ping(ctx, readpref.Primary())
	if err != nil {
		log.Fatalf("[FATAL] Failed to establish a DB connection in a timely manner")
	}
	...
	if err != nil {
		log.Fatalf("[FATAL] Failed to get root user: %s", err)
	}
	if !rootExists {
		// Generate a new random salt
		s, err := security.NewSalt()
		if err != nil {
			log.Fatalf("[FATAL] Failed to generate root user salt: %s", err)
		}
		...
		if err != nil {
			log.Fatalf("[FATAL] Failed to create root user: %s", err)
		}
	}
